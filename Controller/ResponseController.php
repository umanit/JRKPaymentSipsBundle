...
class ResponseController extends Controller
{
    public function autoresponseAction()
    {
        ...
        if ($this->hp('jrk_sips_controller_auto_response')) {
            ...
            $response = $this->forward($this->p("jrk_sips_controller_auto_response"),
                array('response_data' => $structuredData));
        } else {
            // Add result payment in the request
    ...
    public function addResultInLogFile($dataToStore)
    {
        ...
        if (!empty($dataToStore)) {
            ...
            } elseif ($dataToStore["code"] != 0) {
                fwrite($fp, " API call error.\n");
                fwrite($fp, "Error message :  " . $dataToStore["error"] . "\n");
            } else {
                fwrite($fp,
                    "merchant_id : " . $dataToStore['merchant_id'] . "\n");
                fwrite($fp,
                    "merchant_country : " . $dataToStore['merchant_country'] . "\n");
                fwrite($fp, "amount : " . $dataToStore['amount'] . "\n");
                fwrite($fp,
                    "transaction_id : " . $dataToStore["transaction_id"] . "\n");
                fwrite($fp,
                    "transmission_date: " . $dataToStore['transmission_date'] . "\n");
                fwrite($fp,
                    "payment_means: " . $dataToStore['payment_means'] . "\n");
                fwrite($fp,
                    "payment_time : " . $dataToStore['payment_time'] . "\n");
                fwrite($fp,
                    "payment_date : " . $dataToStore['payment_date'] . "\n");
                fwrite($fp,
                    "response_code : " . $dataToStore['response_code'] . "\n");
                fwrite($fp,
                    "payment_certificate : " . $dataToStore['payment_certificate'] . "\n");
                fwrite($fp,
                    "authorisation_id : " . $dataToStore['authorisation_id'] . "\n");
                fwrite($fp,
                    "currency_code : " . $dataToStore['currency_code'] . "\n");
                fwrite($fp,
                    "card_number : " . $dataToStore['card_number'] . "\n");
                fwrite($fp, "cvv_flag: " . $dataToStore['cvv_flag'] . "\n");
                fwrite($fp,
                    "cvv_response_code: " . $dataToStore['cvv_response_code'] . "\n");
                fwrite($fp,
                    "bank_response_code: " . $dataToStore['bank_response_code'] . "\n");
                fwrite($fp,
                    "complementary_code: " . $dataToStore['complementary_code'] . "\n");
                fwrite($fp,
                    "complementary_info: " . $dataToStore['complementary_info'] . "\n");
                fwrite($fp,
                    "return_context: " . $dataToStore['return_context'] . "\n");
                fwrite($fp, "caddie : " . $dataToStore['caddie'] . "\n");
                fwrite($fp,
                    "receipt_complement: " . $dataToStore['receipt_complement'] . "\n");
                fwrite($fp,
                    "merchant_language: " . $dataToStore['merchant_language'] . "\n");
                fwrite($fp, "language: " . $dataToStore['language'] . "\n");
                fwrite($fp,
                    "customer_id: " . $dataToStore['customer_id'] . "\n");
                fwrite($fp, "order_id: " . $dataToStore['order_id'] . "\n");
                fwrite($fp,
                    "customer_email: " . $dataToStore['customer_email'] . "\n");
                fwrite($fp,
                    "customer_ip_address: " . $dataToStore['customer_ip_address'] . "\n");
                fwrite($fp,
                    "capture_day: " . $dataToStore['capture_day'] . "\n");
                fwrite($fp,
                    "capture_mode: " . $dataToStore['capture_mode'] . "\n");
                fwrite($fp, "data: " . $dataToStore['data'] . "\n");
                fwrite($fp,
                    "order_validity: " . $dataToStore['order_validity'] . "\n");
                fwrite($fp,
                    "transaction_condition: " . $dataToStore['transaction_condition'] . "\n");
                fwrite($fp,
                    "statement_reference: " . $dataToStore['statement_reference'] . "\n");
                fwrite($fp,
                    "card_validity: " . $dataToStore['card_validity'] . "\n");
                fwrite($fp,
                    "card_validity: " . $dataToStore['score_value'] . "\n");
                fwrite($fp,
                    "card_validity: " . $dataToStore['score_color'] . "\n");
                fwrite($fp,
                    "card_validity: " . $dataToStore['score_info'] . "\n");
                fwrite($fp,
                    "card_validity: " . $dataToStore['score_threshold'] . "\n");
                fwrite($fp,
                    "card_validity: " . $dataToStore['score_profile'] . "\n");
                fwrite($fp, "-------------------------------------------\n");
            }
        } else {
            fwrite($fp, "No datas\n");
    ...
    public function cancelAction()
    {
        ...
        if ($this->hp('jrk_sips_controller_response')) {
            // Add result payment in the request
            $response = $this->forward($this->p("jrk_sips_cancel_return_url"),
                array('response_data' => $structuredData));
        } else {
            // Add result payment in the request
            $response = $this->forward($this->routeToControllerName($this->p("jrk_sips_cancel_return_url")),
                array('response_data' => $structuredData));
        }
