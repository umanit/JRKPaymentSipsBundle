...
class JRKPaymentSips
{
    ...
    public function get_sips_request(
        $attrbs = array(),
        $transaction = null,
        $tag = null
    ) {
        ...
        $parm .= $this->defset($attrbs, "automatic_response_url",
            $automatic_response_url_route);
        ...
        foreach ($attrbs as $k => $v) {
            $parm .= $k . "=" . $v . " ";
            $this->datas_request[$k] = $v;
        }
    ...
    public function defset(&$arr, $str, $url = false)
    {
        if (!array_key_exists($str, $arr)) {
            if ($url) {
                $val = $this->container->get('router')->generate($this->p("jrk_sips_" . $str),
                    array(), true);
                $this->datas_request[$str] = $val;
                return $str = $str . "=" . $val . " ";
            }
            $val = $this->p("jrk_sips_" . $str);
            if ($str == "currency_code") {
                $val = $this->getCurrencySipsCode($val);
            }
            $this->datas_request[$str] = $val;
            return $str = $str . "=" . $val . " ";
        } elseif ($url) {
            $val = $this->container->get('router')->generate("jrk_sips_" . $arr[$str],
                array(), true);
            $arr[$str] = $val;
            $this->datas_request[$str] = $val;
            return "";
    ...
    public function getCurrencySipsCode($currency_iso)
    {
        ...
        if (!array_key_exists($currency_iso, $currencies)) {
            throw new \Exception("Uknown currency $currency_iso, Known currencies: " . implode(',',
